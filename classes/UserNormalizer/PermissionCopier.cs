...
public sealed class PermissionCopier
{
    ...
    // Add these methods to your existing PermissionCopier class

    public async Task CopyBusinessUnitLegacy(Entity sourceUser, Entity targetUser)
    {
        ArgumentNullException.ThrowIfNull(sourceUser);
        ArgumentNullException.ThrowIfNull(targetUser);
    ...
    public async Task CopyTeamsLegacy(Entity sourceUser, Entity targetUser)
    {
        ArgumentNullException.ThrowIfNull(sourceUser);
        ArgumentNullException.ThrowIfNull(targetUser);

        var sourceTeams = await GetUserTeamsLegacyAsync(sourceUser.Id);
        var targetTeams = await GetUserTeamsLegacyAsync(targetUser.Id);
    ...
    public async Task CopyRolesLegacy(Entity sourceUser, Entity targetUser)
    {
        ArgumentNullException.ThrowIfNull(sourceUser);
        ArgumentNullException.ThrowIfNull(targetUser);

        var sourceRoles = await GetUserRolesLegacyAsync(sourceUser.Id);
        var targetRoles = await GetUserRolesLegacyAsync(targetUser.Id);
        ...
        foreach (var sourceRole in sourceRoles.Entities)
        {
            ...
            var equivalentRole = await FindEquivalentRoleInBusinessUnitLegacyAsync(sourceRole, targetUserBusinessUnit);
    ...
    public async Task<EntityCollection> GetUserTeamsLegacyAsync(Guid userId)
    {
    ...
    public async Task<EntityCollection> GetUserRolesLegacyAsync(Guid userId)
    {
        var query = new QueryExpression("role")
        {
            ColumnSet = new ColumnSet("name"),
    ...
    private async Task<Entity?> FindEquivalentRoleInBusinessUnitLegacyAsync(Entity sourceRole, Guid targetBusinessUnitId)
    {
        ArgumentNullException.ThrowIfNull(sourceRole);
